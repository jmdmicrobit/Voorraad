   ...
   INCLUDE('ABWINDOW.INC'),ONCE
                     ...
                     MAP
                       INCLUDE('VOORRAAD022.INC'),ONCE        !Local module procedure declarations
                     END
...
QuickWindow          WINDOW('Form Versie'),AT(,,370,251),FONT('MS Sans Serif',8,,FONT:regular,CHARSET:DEFAULT), |
  RESIZE,CENTER,GRAY,IMM,MDI,HLP('UpdateVersie'),SYSTEM
                       SHEET,AT(4,4,354,222),USE(?CurrentTab)
                         TAB('&1) General'),USE(?Tab:1)
                           PROMPT('Versie:'),AT(8,28),USE(?VRS:Versie:Prompt),TRN
                           ENTRY(@n-10.2),AT(76,28,48,10),USE(VRS:Versie),READONLY
                           PROMPT('Datum Tijd DATE:'),AT(8,42),USE(?VRS:DatumTijd_DATE:Prompt),TRN
                           ENTRY(@d17),AT(76,42,104,10),USE(VRS:DatumTijd_DATE),READONLY
                           PROMPT('Datum Tijd TIME:'),AT(8,56),USE(?VRS:DatumTijd_TIME:Prompt),TRN
                           ENTRY(@t7),AT(76,56,104,10),USE(VRS:DatumTijd_TIME),READONLY
                           PROMPT('Wijzigingen:'),AT(8,70),USE(?VRS:Wijzigingen:Prompt),TRN
                           TEXT,AT(76,70,269,149),USE(VRS:Wijzigingen,,?VRS:Wijzigingen:2),READONLY,MSG('Aanpassingen in deze versie')
                         END
                       END
                       BUTTON('&Volgende'),AT(240,228,59,14),USE(?OK),LEFT,ICON('WAOK.ICO'),DEFAULT,FLAT,MSG('Accept dat' & |
  'a and close the window'),TIP('Accept data and close the window')
                       BUTTON('&Stoppen'),AT(303,228,55,14),USE(?Cancel),LEFT,ICON('WACANCEL.ICO'),FLAT,MSG('Cancel operation'), |
...
ReturnValue          BYTE,AUTO
  ...
  INIMgr.Fetch('UpdateVersie',QuickWindow)                 ! Restore window settings from non-volatile store
