   ...
   INCLUDE('ABWINDOW.INC'),ONCE
                     ...
                     MAP
                       INCLUDE('VOORRVRD016.INC'),ONCE        !Local module procedure declarations
                     END
...
FDCB10::View:FileDropCombo VIEW(ACel)
                       ...
                       PROJECT(ACel:CelID)
                     END
Queue:FileDropCombo  QUEUE                            !Queue declaration for browse/combo box using ?ACel:CelOms
...
NetLocalDependancies    String('|ViewVoorraadPlanning|ViewArtikel|ViewVoorraadCelTotaal|ViewVoorraadPartij|AViewArtikel|ViewVoorraadPartijTotaal|ViewPartijCelLocaties|ACel|')
QuickWindow          WINDOW('Voorraad'),AT(,,644,279),FONT('MS Sans Serif',8,,,CHARSET:DEFAULT),RESIZE,CENTER,GRAY, |
  ...
  TIP('Close Window')
                       BUTTON('&Verversen Scherm'),AT(497,260,82,14),USE(?Close:2),LEFT,ICON('REFRESH.ICO'),FLAT, |
  MSG('Close Window'),TIP('Close Window')
                       LIST,AT(11,29,450,211),USE(?List:3),FORMAT('60L(2)|M~Partij ID~L(0)@n-14@60L(2)|M~Cel I' & |
  'D~L(1)@n-14@200L(2)|M~Cel Locatienaam~L(0)@s50@60L(2)|M~KG~L(1)@n-14@60L(2)|M~Pallet' & |
  '~L(1)@n-14@'),FROM(Queue:Browse:3),IMM
                     END
  ...
  IF RECORDS(CelVoorraadQ) = 0 THEN
  	...
  	LOOP UNTIL Access:AACel.Next()
  		...
  		LOOP UNTIL Access:AViewVoorraadCelTotaal.Next()
  			db.DebugOut('VulViewVoorraad2 ' & CelCounter# & ' - ' & AVVCT:ArtikelID & ' - ' & (AVVCT:KG - AVVCT:VerkoopKG))
...
FDCB10::View:FileDropCombo VIEW(Cel)
                       ...
                       PROJECT(CEL:CelID)
                     END
Queue:FileDropCombo  QUEUE                            !Queue declaration for browse/combo box using ?CL:Locatienaam
...
ViewPosition           STRING(1024)                   !Entry's view position
                     END
Queue:FileDropCombo:1 QUEUE                           !Queue declaration for browse/combo box using ?CEL:CelOms
