...
QuickWindow          WINDOW('Form Inkoop'),AT(,,593,351),FONT('MS Sans Serif',8,,,CHARSET:DEFAULT),RESIZE,CENTER, |
  ...
  TIP('Cancel operation')
                       PROMPT('Valuta:'),AT(252,176),USE(?Ink:Valuta:Prompt)
                       LIST,AT(289,176,,10),USE(Ink:Valuta),DROP(3),FROM('Euro|#Euro|Dollar|#Dollar|Pond|#Pond')
                       PROMPT('Koersverschil:'),AT(384,176),USE(?Ink:Koersverschil:Prompt)
                       ENTRY(@n-15.3),AT(434,176,60,10),USE(Ink:Koersverschil),RIGHT(2)
                     END
...
ReturnValue          BYTE,AUTO
  ...
  CODE
        udpt.Init(UD,'UpdateInkoop','VoorrPln035.clw','VoorrPln.DLL','06/28/2024 @ 01:22PM')    
  ...
  SELF.AddHistoryField(?Ink:Valuta,30)
  SELF.AddHistoryField(?Ink:Koersverschil,31)
  ...
  IF Ink:Verwerkt=1
      DISABLE(?InkoopPrompt,?Ink:Verwerkt)
      DISABLE(?Insert)
      DISABLE(?Delete)
      UNHIDE(?BtnRetour)
      ENABLE(?BtnRetour)
  ...
  IF SELF.Request = ViewRecord                             ! Configure controls for View Only mode
    ...
    DISABLE(?Delete)
    DISABLE(?Ink:Valuta)
    ?Ink:Koersverschil{PROP:ReadOnly} = True
  END
  BRW7.Q &= Queue:Browse
  ...
  FDCB4.SetFilter('Rel:Type=''S'' AND NOT Rel:NietActief')
  ...
  FDCB4.AddField(Rel:NietActief,FDCB4.Q.Rel:NietActief) !Browse hot field - type derived from field
...
ReturnValue          BYTE,AUTO
  ...
  ReturnValue = PARENT.PrimeUpdate()
    ...
    End
  	If returnValue = Level:Fatal  ! delete just occured     !2020-12-29 deze werkt niet 
  ...
  If event() = event:VisibleOnDesktop !or event() = event:moved
    ds_VisibleOnDesktop()
     ...
     IF KEYCODE()=CtrlShiftP  
        UD.ShowProcedureInfo('UpdateInkoop',UD.SetApplicationName('VoorrPln','DLL'),QuickWindow{PROP:Hlp},'10/06/2011 @ 04:10PM','06/28/2024 @ 01:22PM','10/11/2024 @ 01:55PM')  
