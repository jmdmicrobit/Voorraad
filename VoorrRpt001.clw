   ...
   INCLUDE('ABWINDOW.INC'),ONCE
                     ...
                     MAP
                       INCLUDE('VOORRRPT001.INC'),ONCE        !Local module procedure declarations
                     END
...
!!! </summary>
WindowCallSjabloon PROCEDURE (PRM:SoortData,PRM:RelatieID)
...
Window               WINDOW('Call Sjabloon'),AT(,,230,65),FONT('MS Sans Serif',8),GRAY
  ...
  'ID~L(0)@s19@'),FROM(Queue:FileDropCombo),IMM
                       BUTTON('&OK'),AT(129,46,41,14),USE(?OkButton),LEFT,ICON('WAOk.ico'),DEFAULT
...
ReturnValue          BYTE,AUTO
  ...
  CODE
        udpt.Init(UD,'WindowCallSjabloon','VoorrRpt001.clw','VoorrRpt.DLL','06/20/2018 @ 03:56PM')    
             
  GlobalErrors.SetProcedureName('WindowCallSjabloon')
...
ReturnValue          BYTE,AUTO
  ...
  END
  IF SELF.Opened
    INIMgr.Update('WindowCallSjabloon',Window)             ! Save window data to non-volatile store
  END
      ...
      ThisWindow.Update()
      	If Loc:SjabloonID
      		...
      		of 'W'
                  ...
                  db.Debugout(Loc:Ole{'ActiveDocument.CompatibilityMode'})
                  if Loc:Ole{'ActiveDocument.CompatibilityMode'} = 11
                      Loc:WindowName = Loc:Ole{'ActiveDocument.Name'}&' (Compatibiliteitsmodus) - Microsoft Word'
                  else    
                      Loc:WindowName = Loc:Ole{'ActiveDocument.Name'}&' - Microsoft Word'
                  end    
                  db.Debugout(Loc:WindowName)
...
Loc:KG                  Decimal(10,2)
	CODE
		...
		Loop until Access:Planning.Next()
            ...
            END
			if Loc:Product =''
				Loc:Product = LEFT(Format(Loc:KG,@n-13`2)&' '&Art:ArtikelOms&' '&Ver:VerpakkingOmschrijving)
			ELSE
				Loc:Product = Clip(Loc:Product)&'<13><10>'&LEFT(Format(Loc:KG,@n-13`2)&' '&Art:ArtikelOms&' '&Ver:VerpakkingOmschrijving)
			END
			If Loc:Memo=''
				Loc:Memo = Pla:Memo
			ELSE
	...
	CODE
		...
		Loop until Access:Planning.Next()
			...
			ELSE
				Loc:Memo = Loc:Memo&'<13><10>'&Pla:Memo
            END
            IF GLO:HidePlanningInstructie = 0
                If Loc:Instructie=''
                    Loc:Instructie = Pla:Instructie
                ELSE
                    Loc:Instructie = Loc:Instructie&'<13><10>'&Pla:Instructie
