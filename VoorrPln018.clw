...
QuickWindow          WINDOW('Weging'),AT(,,193,143),FONT('MS Sans Serif',8,,FONT:regular,CHARSET:DEFAULT),DOUBLE, |
  CENTER,GRAY,IMM,HLP('UpdateUitslagMutatieWegingQueue'),SYSTEM
                       PROMPT('Bruto-gewicht (KG):'),AT(12,7),USE(?WGQ:BrutoGewichtKG:Prompt)
                       ENTRY(@n14`2),AT(83,6,60,10),USE(WGQ:BrutoGewichtKG),RIGHT
                       PROMPT('Palletsoort:'),AT(12,22),USE(?PROMPT1)
  ...
  'letsoort~C(0)@s10@32R(2)|M~Tarra (KG)~C(0)@n8`2@'),FROM(Queue:FileDropCombo:1),IMM
                       ...
                       PROMPT('Tijdstip weging:'),AT(12,36),USE(?PROMPT2)
                       PROMPT('Production Date:'),AT(12,54),USE(?WGQ:ProductionDate:Prompt)
                       ENTRY(@d6-),AT(83,54,60,10),USE(WGQ:ProductionDate)
                       PROMPT('THT Date:'),AT(12,70),USE(?WGQ:THTDate:Prompt)
                       ENTRY(@d6-),AT(83,70,60,10),USE(WGQ:THTDate)
                       PROMPT('Harvast Date:'),AT(12,87),USE(?WGQ:HarvastDate:Prompt)
                       ENTRY(@d6-),AT(83,87,60,10),USE(WGQ:HarvastDate)
                       BUTTON('&OK'),AT(82,121,49,14),USE(?Ok),LEFT,ICON('WAOK.ICO'),FLAT,MSG('Accept operation'), |
  TIP('Accept Operation')
                       BUTTON('&Cancel'),AT(135,121,49,14),USE(?Cancel),LEFT,ICON('WACANCEL.ICO'),FLAT,MSG('Cancel Operation'), |
...
ReturnValue          BYTE,AUTO
  ...
  CODE
        udpt.Init(UD,'UpdateUitslagMutatieWegingQueue','VoorrPln018.clw','VoorrPln.DLL','06/28/2024 @ 01:40PM')    
  ...
  If event() = event:VisibleOnDesktop !or event() = event:moved
    ds_VisibleOnDesktop()
     ...
     IF KEYCODE()=CtrlShiftP  
        UD.ShowProcedureInfo('UpdateUitslagMutatieWegingQueue',UD.SetApplicationName('VoorrPln','DLL'),QuickWindow{PROP:Hlp},'10/06/2011 @ 04:10PM','06/28/2024 @ 01:40PM','10/11/2024 @ 01:54PM')  
