   ...
   INCLUDE('BRWEXT.INC'),ONCE
                     ...
                     MAP
                       INCLUDE('VOORRPLN006.INC'),ONCE        !Local module procedure declarations
                     END
...
AAARel:FirmaNaam_Style LONG                           !Field style
...
QuickWindow          WINDOW('Verkoop'),AT(,,667,217),FONT('MS Sans Serif',8,,,CHARSET:DEFAULT),RESIZE,MAXIMIZE, |
  CENTER,GRAY,IMM,MAX,MDI,HLP('BrowseInkoop'),SYSTEM,TIMER(100)
                       LIST,AT(11,58,652,128),USE(?Browse:1),HVSCROLL,FORMAT('45R(2)|*~Verkoop-ID~C(0)@n_10@20' & |
  '0L(2)|M*Y~Afnemer~C(0)@s50@[50R(2)|M*@d6-b@40R(2)|M*@t7b@](77)|~Tijdstip~37C|*I~Verw' & |
  'erkt~@p p@1020L(2)|*~Artikelen~C(0)@s255@'),FROM(Queue:Browse:1),IMM,MSG('Browsing t' & |
  'he Inkoop file')
  ...
  FROM(Queue:FileDropCombo),IMM
                         ...
                         TAB('&6) Verwerkte verkopen per klant'),USE(?TAB3)
                           COMBO(@s50),AT(379,39,157,13),USE(AARel:FirmaNaam),DROP(25),FORMAT('200L(2)|M*~Firma Na' & |
  'am~L(0)@s50@'),FROM(Queue:FileDropCombo:1),IMM
                           BUTTON('...'),AT(539,38),USE(?BUTTON3)
                         END
                         TAB('&7) Gefactureerde verkopen'),USE(?TAB5)
                         END
                         TAB('&8) Gefactureerde verkopen per klant'),USE(?TAB6)
                           COMBO(@s50),AT(292,39,223,13),USE(Loc:Firmanaam3),DROP(25),FORMAT('200L(2)|M*~Firma Naa' & |
  'm~L(0)@s50@'),FROM(Queue:FileDropCombo:2)
                         END
                       END
...
ReturnValue          BYTE,AUTO
  ...
  CODE
        udpt.Init(UD,'BrowseVerkoop','VoorrPln006.clw','VoorrPln.DLL','03/02/2023 @ 02:01PM')    
  ...
  Loc:Timer = GETINI('TIMER','Verkoop',100,PQ:IniFile)
  ...
  BRW1::Sort7:Locator.Init(,Ver2:Planning,,BRW1)           ! Initialize the browse locator using  using key: Ver2:Verkoop_FK3 , Ver2:Planning
  ...
  FDCB5.SetFilter('Rel:Type=''C'' AND NOT Rel:NietActief')
  ...
  FDCB5.AddField(Rel:NietActief,FDCB5.Q.Rel:NietActief) !Browse hot field - type derived from field
  ...
  FDCB2.SetFilter('AAREL:Type=''C'' AND NOT AARel:NietActief')
  ...
  FDCB9.SetFilter('AREL:Type=''C'' AND NOT AREL:NietActief')
  ...
  FDCB9.AddField(AREL:NietActief,FDCB9.Q.AREL:NietActief) !Browse hot field - type derived from field
  ...
  BRW1::SortHeader.Init(Queue:Browse:1,?Browse:1,'','',BRW1::View:Browse,Ver2:PK_Verkoop)
  BRW1::SortHeader.UseSortColors = False
  BRW1::SortHeader.SetStorageSettings('JMDVoorr.ini','BrowseVerkoop')
  BRW1::SortHeader.MultipleColumns = False
  ...
  If event() = event:VisibleOnDesktop !or event() = event:moved
    ds_VisibleOnDesktop()
     ...
     IF KEYCODE()=CtrlShiftP  
        UD.ShowProcedureInfo('BrowseVerkoop',UD.SetApplicationName('VoorrPln','DLL'),QuickWindow{PROP:Hlp},'10/06/2011 @ 04:10PM','03/02/2023 @ 02:01PM','10/11/2024 @ 01:54PM')  
...
BRW1.SetQueueRecord PROCEDURE
  ...
  CODE
  LOC:Artikelen = CachingClass.GetVerkoopArtikelen(Ver2:VerkoopID,Choice(?CurrentTab),Choose(Choice(?CurrentTab) = 4, LOC:Leverancier,Choose(Choice(?CurrentTab) = 6, LOC:Leverancier2,Choose(Choice(?CurrentTab) = 8, LOC:Leverancier3,0))))
  PARENT.SetQueueRecord
  
  IF (AAARel:NietActief)
    SELF.Q.AAARel:FirmaNaam_Style = 1 ! 1
  ELSE
    SELF.Q.AAARel:FirmaNaam_Style = 0 ! 0
  END
  IF (Ver2:Verwerkt)
  ...
  !----------------------------------------------------------------------
  IF Self.q.AAARel:NietActief
     SELF.Q.AAARel:FirmaNaam_NormalBG   = Color:Red
     SELF.Q.AAARel:FirmaNaam_SelectedFG = Color:Red
  END
...
_starttijd              TIME
  ...
  IF ReturnValue
  LogSetSort('Verkoop','NewOrder: '&NewOrder&' Force: '&Force& ' ReturnValue: '&ReturnValue&' '&(Clock()-_starttijd)/100&' sec')
  END
  IF BRW1::LastSortOrder<>NewOrder THEN
     BRW1::SortHeader.ClearSort()
  END
  IF BRW1::LastSortOrder=0 THEN
     BRW1::LastSortOrder=NewOrder
     BRW1::SortHeader.LoadSort()
  END
  BRW1::LastSortOrder=NewOrder
  RETURN ReturnValue
...
FDCB9.SetQueueRecord PROCEDURE
...
BRW1::SortHeader.QueueResorted       PROCEDURE(STRING pString)
  CODE
    ...
    ELSE
       IF CHOICE(?CurrentTab) = 2
          BRW1.ReplaceSort(pString,BRW1::Sort5:Locator)
          BRW1.SetLocatorFromSort()
       ELSIF CHOICE(?CurrentTab) = 7
          BRW1.ReplaceSort(pString,BRW1::Sort6:Locator)
          BRW1.SetLocatorFromSort()
       ELSIF CHOICE(?CurrentTab) = 8
          BRW1.ReplaceSort(pString,BRW1::Sort7:Locator)
          BRW1.SetLocatorFromSort()
       ELSE
