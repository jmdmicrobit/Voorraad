   ...
   INCLUDE('ABREPORT.INC'),ONCE
                     ...
                     MAP
                       INCLUDE('VOORRAAD012.INC'),ONCE        !Local module procedure declarations
                     END
...
ReturnValue          BYTE,AUTO
  ...
  END
  IF SELF.Opened
    INIMgr.Update('ClearPlanning',ProgressWindow)          ! Save window data to non-volatile store
  END
  ProgressMgr.Kill()
...
ReturnValue          BYTE,AUTO
  ...
  ! Overboekingen niet verwijderen
  IF Pla:InkoopID = 0 AND Pla:VerkoopID = 0 THEN RETURN Record:Filtered.
  
  IF Pla:InkoopID > 0
  	CLEAR(Ink:Record)
  	Ink:InkoopID=Pla:InkoopID
