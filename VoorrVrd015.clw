   ...
   INCLUDE('ABWINDOW.INC'),ONCE
                     ...
                     MAP
                       INCLUDE('VOORRVRD015.INC'),ONCE        !Local module procedure declarations
                     END
...
QuickWindow          WINDOW('Partij-Voorraad (View)'),AT(,,609,282),FONT('MS Sans Serif',8,,,CHARSET:DEFAULT),RESIZE, |
  ...
  '@n-13`2@Z(1)57R(2)|M*~Verkocht (KG)~C(0)@n-13`2@Z(1)]|~Cel~61R(2)|M*~Inkoop KG-Prijs' & |
  '~C(0)@n14`3@[40R(2)|M*~Datum ~C(0)@d17@](46)|~Aanmaak~60L(2)|M*~Verpakking~C(0)@s50@' & |
...
LocalClass          CLASS
PartijVoorraadopPeilDatum       Procedure(*CString, Long, *LONG, *REAL, *DECIMAL, *DECIMAL)
                    END
...
!ThisWindow.Export2Excel PROCEDURE(LONG pTab, STRING pSortering) - DATA
  ...
  Of 2
      Loc:Ole{'Sheets("Blad1").Name'} = 'Partijvoorraad_op_Pijldatum'
  End
  
  !omdat de browse op File staat kunnen we de    
  LOC:Rij = 1
  Loc:Ole{'Application.Range(A'&Loc:Rij&').Value'}='Partijvoorraad' & Choose(pTab=2, ' voor pijldatum '&Format(Loc:PeilDatum,@d06-), ' '&Clip(pSortering))
  Do Vet
  ...
  Loc:Ole{'Application.Range(K'&Loc:Rij&').Value'}='Datum'
  Loc:Ole{'Application.Range(L'&Loc:Rij&').Value'}='Verpakking'
  Loc:Ole{'Application.Range(M'&Loc:Rij&').Value'}='Leverancier'
  
  If IGB:BekijkenPrijzen = 1 Then
      Loc:Ole{'Application.Range(N'&Loc:Rij&').Value'}='Inkoop KG-Prijs'
  End
  ...
  LOOP i# = 1 TO Records(BRW2.Q)
      ...
      Loc:Ole{'Application.Range(K'&Loc:Rij&').Value'}=Format(BRW2.Q.VVParI:AanmaakDatum_DATE,@D06-)
      Loc:Ole{'Application.Range(L'&Loc:Rij&').Value'}=BRW2.Q.VVParI:VerpakkingOmschrijving
      Loc:Ole{'Application.Range(M'&Loc:Rij&').Value'}=BRW2.Q.VVParI:LeverancierFirmanaam
      
      If IGB:BekijkenPrijzen = 1 Then
          Loc:Ole{'Application.Range(N'&Loc:Rij&').Value'}=FORMAT(BRW2.Q.VVParI:BerekendeInkoopKGPrijs, @n-13`2)
      End
  ...
  If IGB:BekijkenPrijzen = 1 Then
      Loc:Ole{'Application.Columns(A:N).AutoFit'}
  Else
...
LocalClass.PartijVoorraadopPeilDatum        Procedure(*CString pArtikelID, Long pPartijID, *LONG pVoorraad, *REAL pVerkocht,*DECIMAL pCelVoorraad, *DECIMAL pCelVerkocht)
CODE
	...
	'WHERE dbo.Mutatie.ArtikelID = <39>'&pArtikelID&'<39> AND dbo.Mutatie.PartijID = '&pPartijID&' AND (dbo.Mutatie.DatumTijd is NULL OR dbo.Mutatie.DatumTijd <= ' & CHR(39) & CLIP(FORMAT(LOC:PeilDatum,@d2-)) & ' 23:59' & CHR(39) & ') ' & |
    ...
    END
    pCelVoorraad =pVoorraad
    pCelVerkocht=pVerkocht
...
BRW2.SetQueueRecord PROCEDURE
  ...
  IF CHOICE(?CurrentTab)=2
      ...
      LocalClass.PartijVoorraadopPeilDatum(VVParI:ArtikelID ,VVParI:PartijID , VVParT:VoorraadKG, VPParT:VerkoopKG, LOC:VoorraadKG, VPPar:VerkoopKG)
  ELSE
