   ...
   INCLUDE('ABWINDOW.INC'),ONCE
                     ...
                     MAP
                       INCLUDE('VOORRAAD007.INC'),ONCE        !Local module procedure declarations
                     END
...
QuickWindow          WINDOW('Form Mutatie'),AT(,,244,182),FONT('MS Sans Serif',8,,FONT:regular,CHARSET:DEFAULT), |
  RESIZE,CENTER,GRAY,IMM,MDI,HLP('UpdateRareMutatie'),SYSTEM
                       SHEET,AT(4,4,236,156),USE(?CurrentTab)
                         TAB('&1) General'),USE(?Tab:1)
                           PROMPT('Mutatie ID:'),AT(8,20),USE(?Mut:MutatieID:Prompt),TRN
                           ENTRY(@n-14),AT(76,20,64,10),USE(Mut:MutatieID)
                           PROMPT('Datum Tijd DATE:'),AT(8,34),USE(?Mut:DatumTijd_DATE:Prompt),TRN
                           ENTRY(@d17),AT(76,34,104,10),USE(Mut:DatumTijd_DATE)
                           PROMPT('Datum Tijd TIME:'),AT(8,48),USE(?Mut:DatumTijd_TIME:Prompt),TRN
                           ENTRY(@t7),AT(76,48,104,10),USE(Mut:DatumTijd_TIME)
                           PROMPT('Partij ID:'),AT(8,62),USE(?Mut:PartijID:Prompt),TRN
                           ENTRY(@n-14),AT(76,62,64,10),USE(Mut:PartijID)
                           PROMPT('Cel ID:'),AT(8,76),USE(?Mut:CelID:Prompt),RIGHT,TRN
                           ENTRY(@n-14),AT(76,76,64,10),USE(Mut:CelID),RIGHT(1)
                           PROMPT('Inslag KG:'),AT(8,90),USE(?Mut:InslagKG:Prompt),TRN
                           ENTRY(@n-10`2),AT(76,90,104,10),USE(Mut:InslagKG),DECIMAL(12)
                           PROMPT('Inslag Pallet:'),AT(8,104),USE(?Mut:InslagPallet:Prompt),TRN
                           ENTRY(@n-14),AT(76,104,64,10),USE(Mut:InslagPallet)
                           PROMPT('Uitslag KG:'),AT(8,118),USE(?Mut:UitslagKG:Prompt),TRN
                           ENTRY(@n-10`2),AT(76,118,104,10),USE(Mut:UitslagKG),DECIMAL(12)
                           PROMPT('Uitslag Pallet:'),AT(8,132),USE(?Mut:UitslagPallet:Prompt),TRN
                           ENTRY(@n-14),AT(76,132,64,10),USE(Mut:UitslagPallet)
                           PROMPT('Soort Mutatie:'),AT(8,146),USE(?Mut:SoortMutatie:Prompt),TRN
                           ENTRY(@s4),AT(76,146,40,10),USE(Mut:SoortMutatie)
                         END
                         TAB('&2) General (cont.)'),USE(?Tab:2)
                           PROMPT('Naar CELID:'),AT(8,20),USE(?Mut:NaarCELID:Prompt),RIGHT,TRN
                           ENTRY(@n-14),AT(76,20,64,10),USE(Mut:NaarCELID),RIGHT(1)
                           PROMPT('Reden Afboeking:'),AT(8,34),USE(?Mut:RedenAfboeking:Prompt),TRN
                           ENTRY(@s39),AT(76,34,160,10),USE(Mut:RedenAfboeking)
                           PROMPT('Artikel ID:'),AT(8,48),USE(?Mut:ArtikelID:Prompt),CENTER,TRN
                           ENTRY(@s30),AT(76,48,124,10),USE(Mut:ArtikelID),DECIMAL
                           PROMPT('Planning ID:'),AT(8,62),USE(?Mut:PlanningID:Prompt),CENTER,TRN
                           ENTRY(@n-14),AT(76,62,64,10),USE(Mut:PlanningID),DECIMAL,REQ
                         END
                       END
                       BUTTON('&OK'),AT(85,164,49,14),USE(?OK),LEFT,ICON('WAOK.ICO'),DEFAULT,FLAT,MSG('Accept dat' & |
  'a and close the window'),TIP('Accept data and close the window')
                       BUTTON('&Cancel'),AT(138,164,49,14),USE(?Cancel),LEFT,ICON('WACANCEL.ICO'),FLAT,MSG('Cancel operation'), |
  TIP('Cancel operation')
                       BUTTON('&Help'),AT(191,164,49,14),USE(?Help),LEFT,ICON('WAHELP.ICO'),FLAT,MSG('See Help Window'), |
...
ReturnValue          BYTE,AUTO
  ...
  CODE
        udpt.Init(UD,'UpdateRareMutatie','Voorraad007.clw','Voorraad.EXE','07/01/2024 @ 05:23PM')    
  ...
  INIMgr.Fetch('UpdateRareMutatie',QuickWindow)            ! Restore window settings from non-volatile store
  ...
  If event() = event:VisibleOnDesktop !or event() = event:moved
    ds_VisibleOnDesktop()
     ...
     IF KEYCODE()=CtrlShiftP  
        UD.ShowProcedureInfo('UpdateRareMutatie',UD.SetApplicationName('Voorraad','EXE'),QuickWindow{PROP:Hlp},'07/06/2010 @ 11:12AM','07/01/2024 @ 05:23PM','10/11/2024 @ 01:55PM')  
