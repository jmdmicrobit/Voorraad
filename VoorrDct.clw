...
MainDCT                PROCEDURE   !
     END
     MODULE('VOORRDCT006.CLW')
ExportInvoiceXML       PROCEDURE(<VER:Record>, <INK:Record>, String layout)   !
     END
     MODULE('VOORRDCT007.CLW')
PackLijstClassSource   PROCEDURE   !
LogSetSort             PROCEDURE(String pTabel, String pBericht)   !
     END
     INCLUDE('CWUtil.INC')
     MODULE('win32')
     	OMIT('***',_WIDTH32_)
...
DebugABCGlobalInformation_VoorrDct PROCEDURE()                                           ! DEBUG Prototype
DebugABCGlobalVariables_VoorrDct PROCEDURE()                                             ! DEBUG Prototype
...
Send                PROCEDURE (<STRING p>, long pMyself=1) ,VIRTUAL        
NeedReset           PROCEDURE  (*Long p_Date,*Long p_Time,<*String p_Depend>), VIRTUAL
                            end
   INCLUDE( 'ALSZCLA.INC' ),ONCE
...
GLO:RefreshTijdSlot_Name      string(255) 
...
FilterExclude               CSTRING(4001)                  !                     
FilterDatum                 STRING(8)                      !                     
FilterDatum_GROUP           GROUP,OVER(FilterDatum)        !                     
FilterDatum_DATE              DATE                         !                     
FilterDatum_TIME              TIME                         !                     
                            END                            !                     
                         END
                     END                       

RefreshTijdSlot      FILE,DRIVER('MSSQL','/BUSYHANDLING=2'),OWNER(GLO:Owner),NAME('dbo.RefreshTijdSlot'),PRE(RTS),BINDABLE,THREAD !                     
RefreshTijdSlot_PK       KEY(RTS:Programma,RTS:Werkstation),PRIMARY !                     
Record                   RECORD,PRE()
Programma                   CSTRING(101)                   !                     
Werkstation                 CSTRING(101)                   !                     
Requested                   STRING(8)                      !                     
Requested_GROUP             GROUP,OVER(Requested)          !                     
Requested_DATE                DATE                         !                     
Requested_TIME                TIME                         !                     
                            END                            !                     
TijdSlot                    LONG                           !                     
Booked                      STRING(8)                      !                     
Booked_GROUP                GROUP,OVER(Booked)             !                     
Booked_DATE                   DATE                         !                     
Booked_TIME                   TIME                         !                     
                            END                            !                     
Pulse                       STRING(8)                      !                     
Pulse_GROUP                 GROUP,OVER(Pulse)              !                     
Pulse_DATE                    DATE                         !                     
Pulse_TIME                    TIME                         !                     
                            END                            !                     
Stopped                     STRING(8)                      !                     
Stopped_GROUP               GROUP,OVER(Stopped)            !                     
Stopped_DATE                  DATE                         !                     
Stopped_TIME                  TIME                         !                     
                            END                            !                     
                         END
...
Access:RefreshTijdSlot &FileManager,THREAD                 ! FileManager for RefreshTijdSlot
Relate:RefreshTijdSlot &RelationManager,THREAD             ! RelationManager for RefreshTijdSlot
...
DLLInitializer.Construct PROCEDURE
  ...
  FuzzyMatcher.SetOption(MatchOption:WordOnly, 0)          ! Configure 'word only' matching
   ...
   ! Generated using Clarion Template version v10.0  Family = abc
    ...
    End  !This end
  !Uitgeschakeld en naar voorraad gehaald voor de database keuze
...
ThisNetRefresh.Send      PROCEDURE  (<STRING p>, long pMyself=1) ! Declare Procedure 3
    CODE
   ! UD.Debug('Pre ThisNetRefresh.Send p:'&p&' pMyself:'&pMyself)

      PARENT.Send()
   ! UD.Debug(')
        LogSetSort('Refresh','ThisNetRefresh.Send p:'&p&' pMyself:'&pMyself)
ThisNetRefresh.NeedReset           PROCEDURE  (*Long p_Date,*Long p_Time,<*String p_Depend>)
    CODE

      PARENT.NeedReset(p_Date,p_Time,p_Depend)
      LogSetSort('Refresh','ThisNetRefresh.NeedReset p_Date:'&Format(p_Date,@d6-)&' p_Time:'&Format(p_Time,@t4)&' p_Depend:'&p_Depend)
...
VoorrDct:Init_Called    BYTE,STATIC
  ...
  Access:RefreshTijdSlot.SetErrors(GlobalErrors)
...
!BOE: DEBUG Global
DebugABCGlobalInformation_VoorrDct PROCEDURE()

udpt            UltimateDebugProcedureTracker
                     
  CODE
  
  udpt.Init(UD,'DebugABCGlobalInformation_VoorrDct')
  
 
  RETURN

DebugABCGlobalVariables_VoorrDct PROCEDURE()

udpt            UltimateDebugProcedureTracker

  CODE
  
  udpt.Init(UD,'DebugABCGlobalVariables_VoorrDct')
