...
Loc:TeVerwijderenMutatiesQ QUEUE,PRE(TVQ)                  ! 
PlanningID           LONG                                  ! 
                     END                                   ! 
...
Pla:PlanningIDParent   LIKE(Pla:PlanningIDParent)     !Browse hot field - type derived from field
...
ReturnValue          BYTE,AUTO
  ...
  CODE
        udpt.Init(UD,'UpdateVerkoop','VoorrPln009.clw','VoorrPln.DLL','10/11/2024 @ 12:06PM')    
  ...
  BRW7.AddField(Pla:PlanningIDParent,BRW7.Q.Pla:PlanningIDParent) ! Field Pla:PlanningIDParent is a hot field or requires assignment from browse
  ...
  ! UD.Debug('pre ?List{{PROP:Format}'&?List{PROP:Format})
  ?List{PROP:Format}=GlobalClass.ReplaceKG(?List{PROP:Format})
  !UD.Debug('Na ?List{{PROP:Format}'&?List{PROP:Format})
  ...
  FDCB4.SetFilter('arel:type=''C'' AND NOT AREL:NietActief')
...
ReturnValue          BYTE,AUTO
  ...
  CODE
    IF GlobalRequest=DeleteRecord
        UD.Debug('Pre verwijder check  GlobalRequest:'&GlobalRequest)
        Free(Loc:TeVerwijderenMutatiesQ)    
          ...
          CLEAR(Pla:PlanningID,-1)
          SET(Pla:Planning_FK02,Pla:Planning_FK02)
          LOOP UNTIL Access:Planning.TryNext()
              
              IF Pla:VerkoopID <> Ver2:VerkoopID THEN BREAK.
              
              NoRecord# = false
              IF Pla:MutatieGemaakt
                  TVQ:PlanningID=Pla:PlanningID
                  Add(Loc:TeVerwijderenMutatiesQ)
                  UD.Debug('Te verwijderen planningid '&Pla:PlanningID)
              END
  ...
  ReturnValue = PARENT.Run()
    IF Self.Request=DeleteRecord AND ReturnValue=RequestCompleted
        UD.Debug('Als het goed is is de verkoop verwijderd en de planning, maar niet de gemaakte mutaties :'&Records(Loc:TeVerwijderenMutatiesQ))
        Loop i#=1 TO Records(Loc:TeVerwijderenMutatiesQ)
            Get(Loc:TeVerwijderenMutatiesQ,i#)
            Clear(MUT:Record)
            Mut:PlanningID=TVQ:PlanningID
            Set(Mut:Mutatie_FK02,Mut:Mutatie_FK02)
            Loop Until Access:Mutatie.Next()
                  IF Not Mut:PlanningID=TVQ:PlanningID THEN BREAK.
                  UD.Debug('Delete Planningid '&TVQ:PlanningID&' mutatieid '&Mut:MutatieID)
  ...
  If event() = event:VisibleOnDesktop !or event() = event:moved
    ds_VisibleOnDesktop()
     ...
     IF KEYCODE()=CtrlShiftP  
        UD.ShowProcedureInfo('UpdateVerkoop',UD.SetApplicationName('VoorrPln','DLL'),QuickWindow{PROP:Hlp},'10/06/2011 @ 04:10PM','10/11/2024 @ 12:06PM','10/11/2024 @ 01:54PM')  
...
ReturnValue          BYTE,AUTO
  ...
  IF Request = ChangeRecord AND Pla:MutatieGemaakt THEN
      !Request = ViewRecord
  ELSIF Request = DeleteRecord AND Pla:PlanningIDParent AND  Ver2:Verwerkt = TRUE THEN
      MESSAGE('Deze retour regel mag niet verwijderd worden vanuit dit scherm.|Gebruik hiervoor de Retouren knop. ', 'Retourboeking verwijderen', ICON:Cross)
      RETURN Level:Benign
